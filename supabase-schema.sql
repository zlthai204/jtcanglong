...
-- ============================================================
-- TRỰC XE: điểm danh CA 1 / CA 2 / CA 3 theo từng ngày
-- checked=false/null: chưa kiểm tra; checked=true + present=false: vắng
-- ============================================================
create table if not exists public.sa_vehicle_duty_records (
  id text primary key,
  work_date date not null,
  shift text not null check (shift in ('ca1','ca2','ca3')),
  shipper_name text not null,
  checked boolean not null default false,
  present boolean,
  updated_at timestamptz not null default now(),
  unique(work_date, shift, shipper_name)
);

create index if not exists idx_sa_vehicle_duty_date on public.sa_vehicle_duty_records(work_date);
create index if not exists idx_sa_vehicle_duty_shift on public.sa_vehicle_duty_records(work_date, shift);

alter table public.sa_vehicle_duty_records enable row level security;
drop policy if exists "sa_vehicle_duty_records_anon_all" on public.sa_vehicle_duty_records;
create policy "sa_vehicle_duty_records_anon_all" on public.sa_vehicle_duty_records for all to anon, authenticated using (true) with check (true);


-- Lịch danh sách người trực xe theo tuần (CA 1/2/3)
create table if not exists public.sa_vehicle_duty_roster (
  id text primary key,
  shift text not null check (shift in ('ca1','ca2','ca3')),
...
create index if not exists idx_sa_vehicle_duty_roster_week on public.sa_vehicle_duty_roster(shift,weekday,sort_order);
alter table public.sa_vehicle_duty_roster enable row level security;
drop policy if exists "sa_vehicle_duty_roster_anon_all" on public.sa_vehicle_duty_roster;
create policy "sa_vehicle_duty_roster_anon_all" on public.sa_vehicle_duty_roster for all to anon, authenticated using (true) with check (true);
