...
(function(){
  ...
  async function enableDutyCloud(){
    ...
    try{const fresh=await load(); if(fresh?.payload?.dutyAttendance) window.shipAreaApplyCloud?.({dutyAttendance:fresh.payload.dutyAttendance,dutyRoster:fresh.payload.dutyRoster});}catch(e){console.warn("Tải lại lịch trực xe:",e);}
    return true;
  }
