...
(function(){
  ...
  async function replaceAll(payload){
    ...
    // Không kiểm tra bảng trực xe trong luồng đồng bộ thường.
    // Trực xe là module tùy chọn và chỉ truy cập Supabase khi quản lý chủ động bật Cloud.
    const hasDutyTable=false;
    for(const [table,column] of deletes){
      if(table==='sa_vehicle_duty_records') continue;
