...
(function(){
  ...
  let syncing = false;
  let loading = false;
  let dutyCloudReady = localStorage.getItem("SA_DUTY_TABLE_READY") === "1";
  ...
  function payloadToRows(payload){
    ...
    return {communes,routes,shippers,shipperRoutes,off,setup,entries,entryRoutes,paste,duty};
  }

  async function replaceAll(payload){
    const sb=ensure(true);
    const r=payloadToRows(payload);
    // Giữ cơ chế snapshot cũ cho các chức năng hiện tại của app.
    // Riêng điểm danh trực xe dùng saveDuty() bên dưới để không làm ảnh hưởng dữ liệu khác.
    const deletes=[
      ["sa_vehicle_duty_records","id"],["sa_setup_entry_routes","route_name_snapshot"],["sa_setup_entries","id"],["sa_setup_records","id"],
      ["sa_off_records","id"],["sa_paste_days","commune_name"],["sa_shipper_routes","shipper_id"],["sa_shippers","id"],["sa_routes","id"],["sa_communes","name"]
    ];
    // Kiểm tra bảng trực xe trước khi xóa snapshot. Nếu bảng chưa tồn tại thì vẫn đồng bộ dữ liệu cũ, không được xóa dữ liệu cloud.
    const dutyProbe=await sb.from('sa_vehicle_duty_records').select('id').limit(1);
    const hasDutyTable=!dutyProbe.error;
    for(const [table,column] of deletes){
      if(table==='sa_vehicle_duty_records'&&!hasDutyTable) continue;
      const {error}=await sb.from(table).delete().neq(column,"");
      if(error) throw error;
    }
    const batches=[
      ["sa_communes",r.communes], ["sa_routes",r.routes], ["sa_shippers",r.shippers],["sa_shipper_routes",r.shipperRoutes],["sa_off_records",r.off],["sa_setup_records",r.setup],
      ["sa_setup_entries",r.entries],["sa_setup_entry_routes",r.entryRoutes],["sa_paste_days",r.paste]
    ];
    if(hasDutyTable) batches.push(["sa_vehicle_duty_records",r.duty]);
  ...
  async function enableDutyCloud(){
    ...
    const probe=await sb.from("sa_vehicle_duty_records").select("id").limit(1);
    if(probe.error){
      dutyCloudReady=false; localStorage.removeItem("SA_DUTY_TABLE_READY");
      setStatus("☁️ Chưa có bảng trực xe", "err");
      alert("Supabase chưa có bảng sa_vehicle_duty_records. Hãy chạy phần SQL TRỰC XE trong supabase-schema.sql rồi bấm Cloud lại.");
      return false;
    }
    dutyCloudReady=true; localStorage.setItem("SA_DUTY_TABLE_READY","1");
    setStatus("☁️ Đã bật lưu trực xe", "ok");
    return true;
  }
  ...
  async function load(){
    ...
    try{
      ...
      if(dutyCloudReady) names.push("sa_vehicle_duty_records");
      const res={};
      for(const t of names){
        const {data,error}=await sb.from(t).select("*");
        if(error){
          if(t==='sa_vehicle_duty_records' && /relation .*sa_vehicle_duty_records.*does not exist|Could not find the table|PGRST205/i.test(error.message||'')){res[t]=[];continue;}
          throw error;
        }
        res[t]=data||[];
      }
      ...
      const hasData = Object.keys(areas).length>0 || shippers.length>0 || Object.keys(schedules).length>0 || Object.keys(setupRecords).length>0 || Object.keys(pasteLog).length>0;
      return {
        payload:{areas,shippers,schedules,pasteLog,setupRecords,dutyAttendance},
